...
from base_optimization import base_obj_opt
import numpy as np
...
for d in unique_connect_time_dates[start_date:end_date]: # [start_date:end_date] index represents the date number
    ...
    while( not day_end):
        ...
        if ((sum(SOCdep) > 0) and (need_opt == True) ):
            ...
            TT, I_temp, viz_timev_bat, viz_WEPV, viz_timev_cost, obj1, obj2, obj3,max_TT,peak_per_veh = base_obj_opt(ts_width,soc_init,SOC_xtra,Imax,df,Nv, SOCdep, char_per, SOC_1, del_t,Cbat, opt_time,Vbat)
...
print(len(viz_peak_per_veh) )
print(len(viz_peak_per_veh[0]) )
...
with open('baseline_obj_1_2.txt', 'w') as f:
